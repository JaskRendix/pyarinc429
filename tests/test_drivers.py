...
def fake_serial(monkeypatch):
    fake = FakeSerial()
    monkeypatch.setattr(
        "arinc429.drivers.serial", type("obj", (), {"Serial": FakeSerial})()
    )
    return fake
...
def test_socket_open_close(udp, monkeypatch):
    if udp:
        transport = SocketTransport(
            "127.0.0.1", 0, remote_host="127.0.0.1", remote_port=9999, udp=True
        )
        transport.open()
    else:
        transport = SocketTransport(
            "127.0.0.1", 0, remote_host="127.0.0.1", remote_port=9999, udp=False
        )
