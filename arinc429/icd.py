...
def generate_icd_code(file_path: str | Path) -> str:
    ...
    for item in data.get("labels", []):
        ...
        lines.append(
            f'        """Decode an ARINC 429 Word into {class_name} using ICD field definitions."""'
        )
        ...
        if fields:
            ...
            for field in fields:
                ...
                lines.append(
                    f"        # Field: {field.get('name', fname)} [{ftype}] bits {lsb}..{msb}"
                )
                ...
                if ftype == "BNR":
                    ...
                    if resolution is None:
                        lines.append(
                            f"        {fname} = float(BNR.decode(bits_{fname}, {width}).decoded)"
                        )
                    else:
                        lines.append(
                            f"        {fname} = float(BNR.decode(bits_{fname}, {width}, resolution={resolution}).decoded)"
                        )
                elif ftype == "BCD":
                    # SSM carries the sign/status for BCD data on ARINC 429.
                    if resolution is None:
                        lines.append(
                            f"        {fname} = int(BCD.decode(bits_{fname}, ssm).decoded)"
                        )
                    else:
                        lines.append(
                            f"        {fname} = int(BCD.decode(bits_{fname}, ssm, resolution={resolution}).decoded)"
                        )
                elif ftype == "DISCRETE":
                    lines.append(
                        f"        {fname} = int(Discrete(bits_{fname}).decoded)"
                    )
                else:
                    lines.append(
                        f"        {fname} = bits_{fname}  # Unknown type, raw bits"
                    )
            ...
            lines.append(
                f"        return cls(sdi=sdi, ssm=ssm, raw_data=raw_data, {field_args})"
            )
        else:
    ...
    lines.append(
        '    """Lookup ICD dataclass for the given word.label and decode it, if available."""'
    )
