...
from arinc429.definitions import EQUIP_ADC, EQUIP_ALL, EQUIP_IRS, LabelDefinition
...
def main() -> None:
    ...
    decode_parser.add_argument(
        "raw_word", help="Raw ARINC 429 word in hex (e.g., 0x9c000c26) or decimal."
    )
    decode_parser.add_argument(
        "-p",
        "--profile",
        default="all",
        choices=["adc", "irs", "all"],
        help="Equipment profile.",
    )
    decode_parser.add_argument(
        "--parity", default="odd", choices=["odd", "even"], help="Parity type."
    )
    decode_parser.add_argument(
    ...
    encode_parser = subparsers.add_parser(
        "arinc615-encode", help="Encode payload to ARINC 615 words."
    )
    encode_parser.add_argument(
        "payload", nargs="?", help="String payload to packetize."
    )
    encode_parser.add_argument(
        "-f", "--file", type=Path, help="Read payload from file."
    )
    encode_parser.add_argument(
        "-o", "--output", type=Path, help="Optional output file for JSON words."
    )

    # Williamsburg simulation command
    sim_parser = subparsers.add_parser(
        "williamsburg-simulate", help="Simulate a Williamsburg block transfer."
    )
    sim_parser.add_argument("message", help="Payload message to transfer.")
    sim_parser.add_argument(
        "--trace", action="store_true", help="Print control-word sequence."
    )

    # ICD Load command
    icd_parser = subparsers.add_parser(
        "load-icd", help="Load custom label definitions from an ICD JSON file."
    )
    icd_parser.add_argument(
        "icd_file", type=Path, help="Path to ICD JSON metadata file."
    )

    # ICD Code Generation command
    gen_parser = subparsers.add_parser(
        "generate", help="Generate typed Python dataclasses from an ICD JSON file."
    )
    ...
    gen_parser.add_argument(
        ...
        "--output",
        type=Path,
        help="Output Python file path (defaults to stdout).",
    )

    # Bus simulation command
    sim_bus_parser = subparsers.add_parser(
        "simulate", help="Run a live multi-node ARINC 429 bus simulation."
    )
    sim_bus_parser.add_argument(
        "--duration", type=float, default=2.0, help="Simulation duration in seconds."
    )
    sim_bus_parser.add_argument(
        "--faulty",
        action="store_true",
        help="Introduce a faulty node to test parity and error monitoring.",
    )

    # Replay simulation command
    replay_parser = subparsers.add_parser(
        "replay", help="Replay a recorded JSONL ARINC 429 traffic file."
    )
    replay_parser.add_argument(
        "record_file", type=Path, help="Path to recorded JSONL file."
    )
    replay_parser.add_argument(
    ...
    if args.command == "decode":
        ...
        res, errors = decode_and_validate(
            val, definitions=defs, parity_type=p_type, report_unknown=True
        )
        ...
        if args.json:
            output_data = {
                ...
                "decoded_fields": (
                    {k: str(v) for k, v in decoded.items()} if decoded else {}
                ),
                "errors": errors,
            }
        ...
        print(
            f"Initializing Williamsburg transmitter session with payload: '{args.message}'"
        )
        ...
        try:
            ...
            print(
                f"Successfully loaded {len(loaded)} label definitions from {args.icd_file}"
            )
        except Exception as e:
