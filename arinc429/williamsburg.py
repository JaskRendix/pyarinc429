...
class WilliamsburgSession:
    ...
    def initiate_transfer(self, payload: bytes) -> list[Word]:
        ...
        if not self.is_transmitter:
            raise RuntimeError(
                "Cannot initiate transfer on a receiver-configured session"
            )
    ...
    def process_incoming_word(self, word: Word) -> list[Word] | None:
        ...
        if word.label != self.LABEL_CONTROL:
            if (
                ...
                and word.label == self.LABEL_DATA
            ):
        ...
        if not self.is_transmitter:
            if (
                self.state == WilliamsburgState.IDLE
                and control_code == WilliamsburgControlCode.SAL
            ):
                ...
                self.state == WilliamsburgState.SENT_RTS
                and control_code == WilliamsburgControlCode.SOF
            ):
                self.state = WilliamsburgState.RECEIVING
                return None
                ...
                self.state == WilliamsburgState.RECEIVING
                and control_code == WilliamsburgControlCode.EOF
            ):
                ...
                if (
                    len(self.rx_buffer) < self.expected_length
                    or len(self.rx_buffer) > self.expected_length + 1
                ):
            ...
            if (
                self.state == WilliamsburgState.WAIT_RTS
                and control_code == WilliamsburgControlCode.RTS
            ):
                ...
                words = [
                    self._build_control_word(
                        WilliamsburgControlCode.CTS, len(self.payload)
                    ),
                    self._build_control_word(
                        WilliamsburgControlCode.SOF, len(self.payload)
                    ),
                ]
