    ...
    def on_word_received(self, word: Word, source_id: str) -> None:
        ...
        if word.label == 0o203:
            try:
                ...
                decoded_bnr = BNR.decode(bnr_value=word.data, bnr_bit_length=16, resolution=1.0)
                self.current_altitude = float(decoded_bnr)
                print(f"[{self.node_id}] Altitude from {source_id}: {decoded_bnr} ft (JSON: {decoded_bnr.to_json()})")
        ...
        # Example: Label 0o030 is Active Frequency encoded in BCD
            ...
            try:
                ...
                decoded_bcd = BCD.decode(bcd_value=word.data, bcd_sign=BCD.PLUS, resolution=0.01)
...
def main() -> None:
    ...
    rtu.register_periodic_transmission(
        lambda: WordBuilder().label(0o030).data(0x1218).build(),
        rate_hz=2.0
    )
