...
Shows how to integrate `AsyncBusTransportDriver` with an asynchronous application.

Key points:
- Connects a transport object (serial, socket, or mock) to the ARINC 429 virtual bus.
- Demonstrates inbound hardware traffic being parsed and injected into the bus.
- Demonstrates outbound bus traffic being serialized and written to hardware.
- Useful for hardware‑in‑the‑loop setups, gateway development, and testing custom transports.
